# ALOAM
修改过的ALOAM文件可以在本地运行。
代码生成的轨迹文件是上一级文件夹中的txt文件，在laserMapping.cpp文件void laserOdometryHandler(const nav_msgs::Odometry::ConstPtr &laserOdometry)函数内可以修改文件对应的位置。而且文件每次读写是续写，每次运行之前清空或者移除文件，要么直接修改输出代码。
# A-LOAM
